    ...
    for (let i = 0; i < count; i++) {
      ...
      try {
        ...
        if (photo) {
          ...
          // Save photo incrementally to localStorage and Supabase immediately
          const currentPhotos = getGlobalPhotos();
          if (!currentPhotos.some(p => p.id === photo.id)) {
            const updated = [photo, ...currentPhotos];
                        ...
                        return (
                          <>
                            ...
                            >
                              ...
                              {isUploading ? (
                                <div className="py-2.5 flex flex-col items-center gap-2.5 w-full px-2">
                                  <div className="flex items-center gap-2">
                                    <RefreshCw className={`w-4 h-4 text-brand-gold ${isUploadPaused ? '' : 'animate-spin'}`} />
                                    <span className="text-[10.5px] font-black text-brand-olive uppercase tracking-widest font-serif-display">
                                      {isUploadPaused ? 'Importation en Pause' : `Traitement et Envoi (${batchCompleted} / ${batchTotal})`}
                                    </span>
                                  </div>
                                  
                                  {/* Progress bar */}
                                  <div className="w-full bg-brand-sand/40 h-2.5 rounded-full overflow-hidden relative shadow-inner">
                                    <div 
                                  ...
                                  <div className="flex justify-between w-full text-[9px] font-black uppercase text-brand-sage tracking-wider">
                                  ...
                                  {/* Control Buttons */}
                                  <div className="flex justify-center gap-2 pt-0.5 z-20" onClick={e => e.stopPropagation()}>
                                    <button
                                      type="button"
                                      onClick={() => {
                                        const newPausedState = !isUploadPaused;
                                        setIsUploadPaused(newPausedState);
                                        uploadPausedRef.current = newPausedState;
                                      }}
                                      className="px-2.5 py-1 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 rounded-md text-[8.5px] font-black uppercase tracking-wider transition-all cursor-pointer shadow-xs"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="px-2.5 py-1 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-md text-[8.5px] font-black uppercase tracking-wider transition-all cursor-pointer shadow-xs"
                                    >
                                      ❌ Annuler
                                    </button>
                                  </div>
